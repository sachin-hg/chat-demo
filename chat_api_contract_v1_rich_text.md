  ...
  "properties": {
    ...
    "payload": {
      ...
      "properties": {
        "messageId": {
          "type": "string",
          "description": "Unique ID for this message. Generated by BE for all senders. For bot messages: included directly in the message payload. For user/system messages: BE assigns the ID in the HTTP response to the send-message API call — FE must update its local message object with this ID upon receiving the response."
        },
        ...
        "isFinal": {
          "type": "boolean",
          "description": "true if this is the last message in the response sequence for the current user turn. FE uses this to know the full response has been received. Required when sender.type = bot."
        },
        ...
        "visibility": {
          "type": "string",
          "enum": ["shown", "hidden"],
          "description": "Only meaningful for messageType = user_action. user_action messages are hidden by default (used as silent signals to ML). Set to 'shown' to render derivedLabel as a user bubble in the chat."
        },
...
```json
{
  ...
  "payload": {
    "messageType": "text",
    "responseRequired": true,
    "content": { "text": "can you compare sector 32, sector 21?" }
  }
}
```
---
...
```json
{
  ...
  "payload": {
    "messageId": "msg_009b",
    "sourceMessageId": "src_msg_009",
    "sequenceNumber": 0,
    "isFinal": true,
    "messageType": "markdown",
    "content": {
      "text": "**### Here's all you need to know about sector 21 grugaon.  sector 21 is a bustling localtiy in grugaon with a population of 25K. Few highlights: highlight 1, highlight 2. It has pro1, pro 2. but lacks: con1, con2"
    },
    // [Phase 2] actions — will be implemented in Phase 2
    "actions": [
...
### 4.28.2 User Action login dismissed (can’t proceed)

Use this when an action requires login (shortlist/contact/brochure), but the user closes the login modal without successfully logging in.
