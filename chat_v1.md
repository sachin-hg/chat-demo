...
*(ML → BE `ChatEventFromML` omits `responseRequired`; BE persists to FE as `ChatEventToUser` with `responseRequired: false`.)*
...
- **Every bot message MUST have `messageId`, `sourceMessageId`, `sequenceNumber`, `messageState`, `conversationId`, `createdAt`, and `responseRequired`** (always `false` for bot) on the persisted **`ChatEventToUser`** shape delivered to FE (see §4.3 examples).
...
> **SSE `data` lines:** The `chat_event` payloads above are **abbreviated** for readability. Production **`chat_event`** / **`get-history`** rows use the full **`ChatEventToUser`** object (`conversationId`, `createdAt`, `responseRequired`, …) as in §4.3.
...
**`ChatEventToUser` (bot rows):** Bot examples use the full persisted / wire shape: **`conversationId`**, **`messageId`** (BE-assigned), **`sender`**, **`sourceMessageId`**, **`sequenceNumber`**, **`messageState`**, **`messageType`**, **`createdAt`**, **`responseRequired`** (always `false` for bot), **`content`** — Part B §0 and `ChatEventToUser` in `contract-types.ts`. User and system examples may omit fields not required on the wire where the section title makes the intent obvious.
